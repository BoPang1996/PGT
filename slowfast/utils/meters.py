...
class AVAMeter(object):
    ...
    def __init__(self, overall_iters, cfg, mode, tblogger=None):
        """
            ...
            tblogger (SummaryWriter): tblogger
        """
        self.cfg = cfg
        self.lr = None
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if self.mode == "train":
            ...
            if self.tblogger != None:
                iters = cur_iter + 1 + self.overall_iters * cur_epoch
                for k, v in stats.items():
                    if 'loss' in k:
                        self.tblogger.add_scalar(
                            'train/{}'.format(k), v, iters)
                    elif k == 'eta':
        ...
        if self.mode in ["val", "test"]:
            ...
            if self.tblogger != None:
                for k, v in stats.items():
                    if 'map' in k:
                        self.tblogger.add_scalar(
                            '{}/{}'.format(self.mode, k), v, cur_epoch + 1)
...
class TrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg, tblogger):
        """
        Args:
...
class ValMeter(object):
    ...
    def __init__(self, max_iter, cfg, tblogger):
        ...
        self.num_top5_mis = 0
        self.num_samples = 0
        # Predictions and labels for multi-label tasks
        self.all_preds = []
        self.all_labels = []
        # Tensorboard
        self.tblogger = tblogger
