...
def load_checkpoint(
    ...
    """
    path_to_checkpoint = wrap_input_path2(path_to_checkpoint)
...
def load_test_checkpoint(cfg, model):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        logger.warn("! Load pre-train checkpoint: {}.".format(
...
def load_train_checkpoint(cfg, model, optimizer):
    ...
    """
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.LOGS.DIR):
        last_checkpoint = get_last_checkpoint(cfg.LOGS.DIR)
        logger.info("Load from last checkpoint, {}.".format(last_checkpoint))
