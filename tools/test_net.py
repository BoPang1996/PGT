...
def perform_test(test_loader, model, test_meter, cfg, writer=None, cur_epoch=None):
    ...
    for _, (inputs, labels, video_idx, meta) in enumerate(tqdm(test_loader, **extra_args)):
...
def test(cfg):
    ...
    """
    # Set up environment.
    filename = "test" if cfg.LOGS.FILE_NAME == "" else cfg.LOGS.FILE_NAME
    logging.setup_logger(cfg, filename)
    ...
    logger.info("Testing contains {} videos".format(len(test_loader.dataset)))
